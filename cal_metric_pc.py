...
def load_pancreatic_cancer(exp_name):
...
def cal_metric(file,
                dataset_name: Literal["pancreatic_cancer"],
                model_name: Literal["deepseek-reasoner", "gpt-4.1-mini"]):

    os.environ["data_file"] = f"data/pancreatic_cancer_data_clean.json"
    error_show = False
    specific_ids = []

    if dataset_name == "pancreatic_cancer":
        results, gt_metas, patient_list_id, gt = load_pancreatic_cancer(file)
